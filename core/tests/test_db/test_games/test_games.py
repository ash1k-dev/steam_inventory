...
@pytest.mark.parametrize(
    "test_user_data, test_data_all_games_info,test_data_games,test_data_games_info,test_data_games_count,test_data_games_add",
    LIST_GAMES_CRUD,
)
@pytest.mark.asyncio
async def test_game_crud(
    ...
    await create_steam(
        steam_id=test_user_data["steam_id"],
        telegram_id=test_user_data["telegram_id"],
        steam_name=test_user_data["steam_name"],
        session=session,
    )
    steam = await get_steamid_from_db(
        steam_id=test_user_data["steam_id"], session=session
    )
    await create_all_games(
        all_games_info=test_data_all_games_info, steam_id=steam.id, session=session
    )
    games = await get_games_from_db(
        steam_id=test_user_data["steam_id"], session=session, limit=1000, order="cost"
    )
    assert games == test_data_games
    games_info = await get_games_info_from_db(
        steam_id=test_user_data["steam_id"], session=session
    )
    assert games_info == test_data_games_info
    all_games = await get_all_games_from_db(session)
    assert len(all_games) == test_data_games_count
...
async def update_current_game_cost(game_id, new_cost, session):
    game_from_db = await get_game_from_db(game_id, session)
    game_from_db.cost = new_cost
    session.add(game_from_db)
    await session.commit()


@pytest.mark.parametrize(
    "test_user_data, test_all_games_data,test_all_tracking_games_long,test_tracking_games_changes",
    LIST_TRACKING_GAMES_CRUD,
)
@pytest.mark.asyncio
async def test_game_track_crud(
    test_user_data,
    test_all_games_data,
    test_all_tracking_games_long,
    test_tracking_games_changes,
    get_game_name_mock,
    get_game_cost_mock,
    session,
):
    ...
    for game_id, game_data in test_all_games_data.items():
        await create_game_track(
            game_id=game_id, telegram_id=test_user_data["telegram_id"], session=session
        )
    ...
    all_tracking_games = await get_all_tracking_games_from_db(
        telegram_id=test_user_data["telegram_id"], session=session
    )
    assert len(all_tracking_games) == test_all_tracking_games_long
    for game_id, game_data in test_all_games_data.items():
        await update_current_game_cost(
            game_id=game_id, new_cost=game_data["cost"], session=session
        )
    tracking_games_changes = await get_tracking_games_changes(
        telegram_id=test_user_data["telegram_id"], session=session
    )
    assert tracking_games_changes == test_tracking_games_changes
    for game_id in test_all_games_data:
